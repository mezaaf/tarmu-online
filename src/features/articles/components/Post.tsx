      ...
      {props.articles?.map((item) => (
        <div key={item.id} className="flex w-xs flex-col md:w-3xl">
          ...
          <div className="mt-5 mb-5">
            <Link href={`/articles/${item.category.name}/${item.slug}`}>
              <InteractiveHoverButton className="border-tosca hover:bg-tosca cursor-pointer rounded-full bg-transparent text-xs transition-all duration-300 hover:text-white">
                Selengkapnya
